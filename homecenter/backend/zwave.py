...
class Zwave:
    ...
    def options(self):
        ...
        try:
            options = ZWaveOption(settings.DEVICE,
    ...
    def start(self):
        if self.network.is_ready:
            ...
            print("***** Le réseau est en cours de démarrage. "
                  "Veuillez patienter. *****")
            for i in range(0, 90):
                if self.network.state >= self.network.STATE_READY:
                    print("***** Le réseau est prêt ! *****")
                    print("Le controlleur réseau est : {}".
                          format(self.network.controller))
    ...
    def stop(self):
        if self.network.is_ready:
            print("***** Le réseau est en cours de d'arrêt. "
                  "Veuillez patienter. *****")
            self.network.stop()
            stop_listener()
        else:
